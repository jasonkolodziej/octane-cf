...
7. **Drizzle: db not initialized** - `drizzleAdapter(db, ...)` requires a `db` instance from `drizzle()`. See [references/create-auth.md](create-auth.md) for setup examples (node-postgres, postgres.js, Neon).
